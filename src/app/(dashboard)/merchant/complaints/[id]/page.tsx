...
export default function MerchantComplaintDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const queryClient = useQueryClient()
  const [response, setResponse] = useState('')

  const { data, isLoading, error } = useQuery({
    queryKey: ['merchant-complaint', id],
    queryFn: async () => {
  ...
  const respondMutation = useMutation({
    mutationFn: async (body: { response: string }) => {
      ...
      const json = await res.json()
      if (!res.ok) throw new Error(json.error?.message ?? 'Failed to send response')
      return json
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['merchant-complaint', id] })
      setResponse('')
      showToast({ type: 'success', title: 'Response sent', description: 'Your response has been recorded.' })
    },
  ...
  if (error || !data?.data) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <Link href="/merchant/complaints" className="mt-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <textarea
              ...
              placeholder="Provide your response or clarification..."
            />
            <div className="flex justify-end">
              <Button onClick={handleRespond} disabled={respondMutation.isPending}>
                {respondMutation.isPending ? (
                  ...
                  <><MessageSquare className="mr-1 h-4 w-4" /> Submit Response</>
                )}
              </Button>
            </div>
          </CardContent>
